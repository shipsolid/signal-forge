...
Write structured JSON to stdout — do **not** export logs via OTLP. Alloy's `loki.source.kubernetes` pipeline tails pod logs and ships them to Loki. See [ADR-001](https://shipsolid.github.io/signal-forge/architecture/adrs/adr-log-tailing-not-otlp-export/) for the rationale.
...
See [Log-to-Trace Correlation](https://shipsolid.github.io/signal-forge/observability/correlation/) for the Alloy stage configuration.
...
See [OTel Signal Contracts](https://shipsolid.github.io/signal-forge/observability/otel-contracts/) for the full attribute catalogue.
...
All services must have automated tests that run without a cluster. See [Testing](https://shipsolid.github.io/signal-forge/testing/) for the test strategy and isolation patterns.
...
- **Fail-fast at startup** for required configuration (DB connection strings, downstream
  ...
  developer's machine. See [ADR-006](https://shipsolid.github.io/signal-forge/architecture/adrs/adr-fail-fast-on-missing-secrets/)
...
- **Validate at the API boundary**, not deeper in the call chain — return a structured `422`
  ...
  [Security § Input validation](https://shipsolid.github.io/signal-forge/operations/security/#input-validation)
...
- **Never interpolate exception content into a log message string.** Use `logger.exception()`
  ...
  See [Security § Credential leakage prevention](https://shipsolid.github.io/signal-forge/operations/security/#credential-leakage-prevention-in-logs).
- **Distinguish transient failures from poison input** on any consumer/retry path — a downstream
  ...
  [Resilience Patterns](https://shipsolid.github.io/signal-forge/operations/resilience-patterns/) for the pattern
