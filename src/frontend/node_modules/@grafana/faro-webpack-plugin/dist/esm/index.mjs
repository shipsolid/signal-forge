...
import fs from 'fs';
import { randomString, THIRTY_MB_IN_BYTES, exportBundleIdToFile, faroBundleIdSnippet, normalizePrefix, WEBPACK_PLUGIN_NAME, consoleInfoOrange, shouldProcessFile, uploadCompressedSourceMaps, uploadSourceMap } from '@grafana/faro-bundlers-shared';
    ...
    if (filename.endsWith('.map')) {
      try {
        ...
        if (sourceMap.file && !sourceMap.file.startsWith(normalizedPrefix)) {
          ...
          compilation.updateAsset(filename, new sources.RawSource(JSON.stringify(sourceMap)));
          verbose && consoleInfoOrange(`Modified source map file property: ${filename} -> ${sourceMap.file}`);
        }
      } catch (error) {
...
class FaroSourceMapUploaderPlugin {
  constructor(options) {
    ...
    this.bundleId = (_a = options.bundleId) !== null && _a !== void 0 ? _a : String(Date.now() + randomString(5));
    ...
    this.maxUploadSize = options.maxUploadSize && options.maxUploadSize > 0 ? options.maxUploadSize : THIRTY_MB_IN_BYTES;
...
export { FaroSourceMapUploaderPlugin as default };
