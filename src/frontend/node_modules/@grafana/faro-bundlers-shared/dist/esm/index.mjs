 ...
 */
const ensureSourceMapFileProperty = (filePath, verbose) => {
  try {
    ...
    if (!sourceMap.file) {
      ...
      verbose && consoleInfoOrange(`Added file property to source map: ${mapFileName} -> file: ${jsFileName}`);
    }
  } catch (error) {
    console.error(`Error ensuring source map file property ${filePath}:`, error instanceof Error ? error.message : String(error));
  }
};
/**
 ...
 */
const ensureSourceMapFileProperties = (outputDir, verbose, recursive) => {
  const walk = dir => {
    const results = [];
    ...
    for (const filePath of allFiles) {
      ...
      try {
        ...
        if (jsName) {
          ...
          fs.writeFileSync(filePath, JSON.stringify(sourceMap));
          verbose && consoleInfoOrange(`Set file property on source map: ${mapBasename} -> file: ${jsName}`);
        } else {
          // Fallback: derive from map filename
          const jsFileName = mapBasename.replace(/\.map$/, "");
          sourceMap.file = jsFileName;
          fs.writeFileSync(filePath, JSON.stringify(sourceMap));
          verbose && consoleInfoOrange(`Added file property to source map: ${mapBasename} -> file: ${jsFileName}`);
        }
      } catch (error) {
        console.error(`Error processing source map ${filePath}:`, error instanceof Error ? error.message : String(error));
      }
    }
  } catch (error) {
    console.error(`Error ensuring source map file properties in ${outputDir}:`, error instanceof Error ? error.message : String(error));
  }
};
/**
 ...
 */
const modifySourceMapFileProperty = (filePath, prefix, verbose, basenameOnly) => {
  try {
    ...
    if (sourceMap.file && !sourceMap.file.startsWith(normalizedPrefix)) {
      ...
      fs.writeFileSync(filePath, JSON.stringify(sourceMap));
      verbose && consoleInfoOrange(`Modified source map file property: ${path.basename(filePath)} -> ${sourceMap.file}`);
    }
  } catch (error) {
    console.error(`Error modifying source map file ${filePath}:`, error instanceof Error ? error.message : String(error));
  }
};
crypto.randomUUID();

export { ESBUILD_PLUGIN_NAME, JS_SOURCEMAP_PATTERN, ROLLUP_PLUGIN_NAME, THIRTY_MB_IN_BYTES, WEBPACK_PLUGIN_NAME, cleanAppName, consoleInfoOrange, ensureSourceMapFileProperties, ensureSourceMapFileProperty, exportBundleIdToFile, faroBundleIdSnippet, modifySourceMapFileProperty, normalizePrefix, randomString, shouldProcessFile, uploadCompressedSourceMaps, uploadSourceMap, validateProxyUrl };
