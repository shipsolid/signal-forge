...
Base URL: `http://localhost:8080/api` (via Traefik ingress)
Internal cluster URL: `http://gateway-api.otel-lab.svc.cluster.local:5000`

All endpoints accept and return `application/json`. Error responses follow the RFC 7807 Problem Details format.
...
Create an order. Triggers gRPC call to order-api, which persists the order and publishes an `order.created` event to RabbitMQ.
...
The trace for this request will have `otel.status_code=ERROR` and an `exception.stacktrace` span event.
...
All API endpoints accept and propagate the W3C `traceparent` header. When the Angular SPA sends a request with this header, the gateway-api HTTP server span becomes a child of the browser span.
