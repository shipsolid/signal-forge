...
- [ ] Grafana panel has "Exemplars" toggle enabled and a "Data link" pointing to the Jaeger datasource

## Why `OTEL_METRICS_EXEMPLAR_FILTER` instead of `AddExemplarFilter()`

The SDK method `AddExemplarFilter(ExemplarFilterType.TraceBased)` is behind an experimental flag in OTel .NET SDK 1.9.x. It requires:
...
The `/api/slow` endpoint always runs inside a sampled span (it's always kept by tail sampling) and always records a histogram observation. Use it to reliably generate exemplars:
...
1. Open a Grafana dashboard panel showing `http_server_request_duration_seconds` or `traces_spanmetrics_latency_bucket`
2. Edit the panel → Query → enable **Exemplars** toggle
3. Panel → Options → Data links → add link: type = Jaeger datasource, URL = `${__value.raw}` (the raw traceId)
...
No additional configuration is needed; exemplars are part of the OTLP protobuf payload and are forwarded by Alloy's `otelcol.exporter.otlphttp` exporter.
