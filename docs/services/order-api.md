...
relations:
  - slug: projects/app-signal-forge/api/grpc
    kind: depends_on
  - slug: projects/app-signal-forge/architecture/adrs/adr-grpc-streaming-via-asasyncenumerable
    kind: depends_on
...
**Role**: [[networks/05-http-ecosystem/05-grpc/05-grpc|gRPC]] Order Service. Handles order CRUD,
persists to PostgreSQL, publishes `order.created` events to RabbitMQ.

**Runtime**: .NET 8 gRPC server (+ minimal API for `/healthz`) **Ports**: 5001 (HTTP/1.1, `/healthz`
only — kubelet's probes can't negotiate HTTP/2), 5002 (gRPC, HTTP/2-only, cluster-internal)
**Replicas**: 2
...
Kestrel logs "HTTP/2 requires TLS application protocol negotiation" and rejects gRPC's
prior-knowledge h2c preface with an HTTP_1_1_REQUIRED error). See `Program.cs`'s "gRPC server"
...
(see [[adr-grpc-streaming-via-asasyncenumerable|ADR-010]]).
