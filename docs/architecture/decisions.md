...
- At production scale, log volume spikes must not consume SDK/process memory or CPU. Node-level
  agents absorb backpressure independently.
...
context, not a parent-child relationship.
...
reference artifact but is not deployed.
...
in both configs — the trace-ID/span-ID → Loki structured-metadata correlation stages — is now
authored once at `k8s/monitoring/grafana/shared/trace-correlation-stages.alloy` and spliced into
both `configmap.yaml.tmpl` (raw) and `values-cloud.yaml.tmpl` (Helm-`tpl`-escaped) by
...
- The dead-letter pattern is built into RabbitMQ — no additional application code is needed in the
  NACK path. The consumer NACKs with `requeue=False`; the broker handles routing.
...
in Alloy, not by the application SDK.
...
**Decision**: `GetOrdersByProject` streams rows directly from the PostgreSQL cursor using
`AsAsyncEnumerable()`, writing each row to the gRPC stream as it is fetched.
