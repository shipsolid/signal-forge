...
`deploy-local.sh` (see [[projects/app-signal-forge/deployment/local|Local Deployment]]) sources the
...
consumers) and `monitoring` ([[projects/app-signal-forge/deployment/helm|Helm chart]]'s Alloy).
