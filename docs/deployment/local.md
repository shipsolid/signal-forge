...
fetches/applies Grafana Cloud credentials (see [grafana-cloud.md](grafana-cloud.md)).
...
See [CLAUDE.md](../../CLAUDE.md) for the full flag list and the safety checks `deploy-local.sh` runs
before every apply (k3d context guard, NodePort drift check, Secret-key contract validation).
...
`--with-helm` is passed in local mode. See [helm.md](helm.md) for the Alloy role breakdown.
