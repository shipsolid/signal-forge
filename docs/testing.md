...
| End-to-end trace propagation                                                                          | Automated by `src/integration-tests` (opt-in, needs Docker — see above); also validated manually via [[tech/jaeger\|Jaeger]] UI ([[projects/app-signal-forge/spec\|docs/spec.md]] checklist) for exploratory checks |
